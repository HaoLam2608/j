...
CREATE TRIGGER TuDongPhanQuyen
ON NhanVien
AFTER INSERT
AS
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM sys.server_principals WHERE NAME = @USERNAME AND type_desc = 'SQL_LOGIN')
    BEGIN
        -- Create the SQL login dynamically
    ...
    SELECT @TENNHOM = TENNHOM
    FROM NHOMQUYEN WHERE MANHOM = @MANHOM

    IF @TENNHOM = N'ThuNgan'
    BEGIN
        SET @Sql = 'EXEC sp_addrolemember ''ThuNgan'', ''' + @USERNAME + ''';'
        EXEC sp_executesql @Sql;
    END
    ELSE IF @TENNHOM = N'Quanly'
    BEGIN
        SET @Sql = 'EXEC sp_addrolemember ''Quanly'', ''' + @USERNAME + ''';'
        EXEC sp_executesql @Sql;
    END
    ELSE
    BEGIN
        SET @Sql = 'EXEC sp_addrolemember ''Kho'', ''' + @USERNAME + ''';'
        EXEC sp_executesql @Sql;
    END
END
...
VALUES 
('NV00023', N'lâm hào', 1, '0912345679', N'123 Đường Lý Tự Trọng, Quận 1, TP.HCM', 1, 'NHOM01', '123456', 'haolam')
...
GRANT SELECT, INSERT, UPDATE, DELETE on DSNGUYENLIEU to Kho;
GRANT SELECT, INSERT, UPDATE, DELETE on VIEW_4 to Kho;
GRANT SELECT, INSERT, UPDATE, DELETE on VIEW_5 to Kho;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON fn_ViewAllInvoices TO ThuNgan;
GRANT SELECT, INSERT, UPDATE, DELETE on vw_CustomerInfo to ThuNgan

-------------Quyền quản lý----------------
exec sp_addlogin 'nv00001','123456'
exec sp_adduser 'nv00001','nv00001'
exec sp_addrolemember QuanLy,'nv00001'
go
use master
GRANT ALTER ANY LOGIN TO nv00001
go 
use QLCF
...
GRANT SELECT, INSERT, UPDATE, DELETE ON fn_ViewAllInvoices TO QuanLy;
GRANT SELECT, INSERT, UPDATE, DELETE on DSNGUYENLIEU to QuanLy
GRANT SELECT, INSERT, UPDATE, DELETE on VIEW_4 to QuanLy
GRANT SELECT, INSERT, UPDATE, DELETE on VIEW_5 to QuanLy
GRANT SELECT, INSERT, UPDATE, DELETE on vw_CustomerInfo to QuanLy
GRANT SELECT, INSERT, UPDATE, DELETE on vw_ThongTinNhanVienChiNhanh to QuanLy
GRANT SELECT, INSERT, UPDATE, DELETE on XEMDSNV to QuanLy
GRANT exec on fn_CreateMaNhanVien to QuanLy
